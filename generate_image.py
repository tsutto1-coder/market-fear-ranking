...
Y_SECTION = HEADER_H
Y_ROW1    = Y_SECTION + SECTION_H
Y_ROW2    = Y_ROW1 + ROW1_H
Y_ROW3    = Y_ROW2 + ROW2_H
Y_CHART   = Y_ROW3 + ROW3_H
Y_BOTTOM  = Y_CHART + CHART_H

NIKKEI225_TICKERS = {
    '7203.T':'トヨタ自動車',   '6758.T':'ソニーグループ',
    '9984.T':'ソフトバンクG',  '8306.T':'三菱UFJ FG',
    '6501.T':'日立製作所',     '6861.T':'キーエンス',
    '4063.T':'信越化学工業',   '8035.T':'東京エレクトロン',
    '7974.T':'任天堂',         '6367.T':'ダイキン工業',
    '4519.T':'中外製薬',       '9432.T':'NTT',
    '9433.T':'KDDI',           '9434.T':'ソフトバンク',
    '7267.T':'ホンダ',         '7269.T':'スズキ',
    '7751.T':'キヤノン',       '6702.T':'富士通',
    '4661.T':'オリエンタルランド','2802.T':'味の素',
    '2914.T':'JT',             '3382.T':'セブン&アイHD',
    '8801.T':'三井不動産',     '8802.T':'三菱地所',
    '9020.T':'JR東日本',       '5401.T':'日本製鉄',
    '4503.T':'アステラス製薬', '4568.T':'第一三共',
    '6971.T':'京セラ',         '8316.T':'三井住友FG',
    '8411.T':'みずほFG',       '7832.T':'バンダイナムコ',
    '6098.T':'リクルートHD',   '6954.T':'ファナック',
    '7011.T':'三菱重工業',     '6301.T':'小松製作所',
    '6326.T':'クボタ',         '5802.T':'住友電工',
    '3407.T':'旭化成',         '4452.T':'花王',
    '5108.T':'ブリヂストン',   '7013.T':'IHI',
    '4689.T':'LY Corporation', '9022.T':'JR東海',
}
...
def _cx(draw, text, y, f, color, gap=10, max_w=W-80):
    ...
    for raw in text.split('\n'):
        ...
        line = ''
        for ch in raw:
            if draw.textlength(line+ch, font=f) > max_w and line:
                lines.append(line); line = ch
            else: line += ch
        lines.append(line)
    for ln in lines:
        bx = draw.textbbox((0,0), ln, font=f); lw = bx[2]-bx[0]
        draw.text(((W-lw)//2, y), ln, font=f, fill=color)
        y += bx[3]-bx[1]+gap
    return y
...
SYSTEM_PROMPT = (
    'あなたは日本株市場のデータを教育目的で解説するコンテンツ作成者です。\n'
    '【禁止】売買推奨・将来予測の断定\n'
    '【出力】以下JSONのみ。\n'
...
def generate_script(ranked, fear_greed):
    ...
    valid  = [s for s in ranked if not s.get('empty')]
    if not valid:
        return {
            'summary': '本日は条件に合致する銘柄がありませんでした。',
            'hashtags': ['日本株', '投資初心者', 'AI分析', '市場心理']
        }
    stocks_text = '\n'.join(
...
def make_small_chart(dates, closes, rsi_values, width=988, height=228, dpi=150):
    ...
    a1.annotate(f'{closes[-1]:,.0f}', (dates[-1], closes[-1]),
        ...
        color='#DC3C3C', fontsize=9, fontweight='bold')
...
def draw_row(draw, img, rank, stock, y, row_h, is_first=False):
    ...
    score_cols = [RED, (145,150,168), (88,92,108)]
    bar_fills  = [RED, (62,67,85),    (40,44,60)]
    bc, btxt   = badge_cols[rank-1]

    # ── 「ありませんでした」行 ────────────────────────
    if stock.get('empty'):
        row_bg = (19,15,15) if is_first else ((17,20,28) if rank==2 else (15,17,25))
        draw.rectangle([(0,y),(W,y+row_h)], fill=row_bg)
        draw.line([(40,y),(W-40,y)], fill=(32,36,52), width=1)
        BR = 36 if is_first else 30; BX = 48+BR; BY = y+row_h//2
        draw.ellipse([(BX-BR,BY-BR),(BX+BR,BY+BR)], fill=bc)
        badge_f = _font(FONT_B, 40 if is_first else 34)
        rt = str(rank); rb = draw.textbbox((0,0), rt, font=badge_f)
        draw.text((BX-(rb[0]+rb[2])//2, BY-(rb[1]+rb[3])//2), rt, font=badge_f, fill=btxt)
        msg_f = _font(FONT_R, 34 if is_first else 28)
        msg   = 'ありませんでした'
        mb    = draw.textbbox((0,0), msg, font=msg_f)
        draw.text((BX+BR+24, BY-(mb[3]-mb[1])//2), msg, font=msg_f, fill=(70,75,90))
        return

    # ── 通常行 ───────────────────────────────────────
    sc = score_cols[rank-1]; bf = bar_fills[rank-1]
    if is_first:
        draw.rectangle([(0,y),(W,y+row_h)], fill=(19,15,15))
        draw.rectangle([(0,y),(4,y+row_h)], fill=RED)
    else:
        draw.rectangle([(0,y),(W,y+row_h)], fill=((17,20,28) if rank==2 else (15,17,25)))
    draw.line([(40,y),(W-40,y)], fill=(32,36,52), width=1)
    ...
    pc=stock['price_change']; pc_str=f'{pc:+.1f}%'
    rsi_str=f'{stock["rsi"]:.1f}'; ticker=stock['ticker'].replace('.T','')
    pc_clr=(210,55,55) if pc<0 else (55,190,100)
    cx=TX
    draw.text((cx,meta_y),ticker,font=meta_f,fill=GRAY)
    cx+=draw.textbbox((0,0),ticker,font=meta_f)[2]+16
    draw.text((cx,meta_y),'｜',font=meta_f,fill=(50,55,70))
    cx+=draw.textbbox((0,0),'｜',font=meta_f)[2]+10
    draw.text((cx,meta_y+4),'前日比',font=label_f,fill=GRAY)
    cx+=draw.textbbox((0,0),'前日比',font=label_f)[2]+8
    draw.text((cx,meta_y),pc_str,font=meta_f,fill=pc_clr)
    cx+=draw.textbbox((0,0),pc_str,font=meta_f)[2]+16
    draw.text((cx,meta_y),'｜',font=meta_f,fill=(50,55,70))
    cx+=draw.textbbox((0,0),'｜',font=meta_f)[2]+10
    draw.text((cx,meta_y+4),'RSI',font=label_f,fill=GRAY)
    cx+=draw.textbbox((0,0),'RSI',font=label_f)[2]+8
...
def build_image(ranked, fear_greed, script):
    ...
    if not ranked[0].get('empty'):
        d.text((44,Y_CHART+10),
               f'1位 {ranked[0]["name"]} チャート（30日）',
               font=_font(FONT_B,26), fill=GRAY)
        chart=make_small_chart(ranked[0]['dates'],ranked[0]['closes'],
                               ranked[0]['rsi_series'],width=988,height=228,dpi=150)
        img.paste(chart.resize((988,228),Image.LANCZOS).convert('RGB'),
                  ((W-988)//2,Y_CHART+42))
    else:
        _cx(d,'本日は条件に合致する銘柄がありませんでした',
            Y_CHART+CHART_H//2-30, _font(FONT_R,34),(70,75,90))
    ...
    next_y=_cx(d,'  '.join(f'#{h}' for h in script['hashtags']),
               next_y+8,_font(FONT_R,29),(72,120,190))
    _cx(d,'本コンテンツは教育目的の情報提供です。投資助言ではありません。',
        next_y+12,_font(FONT_R,23),(56,60,76))
